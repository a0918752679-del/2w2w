...
function caseFromFlatRow(type, row) {
  ...
  const item = {
    ...
    admin: {
      ...
      officialDocumentNo: normalizeText(row['公文案號']),
      officialDocumentDate: normalizeText(row['公文日期']),
      dispositionNo: normalizeText(row['裁處書號']),
      penaltyAmount: normalizeText(row['裁罰金額']) || normalizeText(row['金額']),
      penaltyProgress: normalizeText(row['裁罰進度']) || '未裁處',
      deliveryStatus: normalizeText(row['送達狀態']) || '未送達',
      paymentStatus: normalizeText(row['繳款狀態']) || '未開立',
...
function toFlatRow(item) {
  ...
  row['公文案號'] = item.admin?.officialDocumentNo || '';
  row['公文日期'] = item.admin?.officialDocumentDate || '';
  row['裁處書號'] = item.admin?.dispositionNo || '';
  row['裁罰金額'] = item.admin?.penaltyAmount || '';
  row['裁罰進度'] = item.admin?.penaltyProgress || '';
  row['送達狀態'] = item.admin?.deliveryStatus || '';
...
function upsertCases(existing, incoming) {
  ...
  return { cases: Array.from(map.values()), inserted, updated };
}
...
app.get('/api/cases/:id', (req, res) => {
  const item = readCases().find((c) => c.id === req.params.id);
...
app.patch('/api/cases/:id/admin', (req, res) => {
  ...
  const allowed = [
    ...
    'penaltyAmount', 'penaltyProgress', 'deliveryStatus', 'paymentStatus',
  ...
  if (item.admin.closeDate || item.admin.stage === '已結案' || item.admin.penaltyProgress === '結案' || item.admin.inspectionStatus === '結案') {
    item.status = '已結案';
  ...
  addLog(item, '更新行政流程', normalizeText(req.body.operator) || '承辦人員', item.admin.notes || '');
  writeCases(cases);
  res.json({ ok: true, item });
});
...
app.post('/api/sync/pull', async (req, res) => {
  try {
    ...
    const result = upsertCases(readCases(), incoming);
    writeCases(result.cases);
    res.json({ ok: true, pulled: incoming.length, inserted: result.inserted, updated: result.updated });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});
